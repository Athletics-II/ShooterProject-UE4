	...
	FHitResult& OutHitResult)
{
	...
	if (!OutHitResult.bBlockingHit) // is there an obj between barrel and beamendpoint?
	{
	...
	if (bShouldTraceForItems)
	{
		...
		if (ItemTraceResult.bBlockingHit)
		{
			...
			if (TraceHitWeapon)
			{
				...
				{
					// Not currently highlighting a slot; start highlighting
					HighlightInventorySlot();
				}
			}
			else
			{
				// Check if a slot is being highlighted already
				if (HighlightedSlot != -1)
				{
...
void AShooterCharacter::EquipWeapon(AWeapon* WeaponToEquip)
{
	if (WeaponToEquip)
	{
		...
		{
			// -1 == no equippedweapon yet, no need to reverse icon animation
...
void AShooterCharacter::SendBullet()
{
	...
	if (BarrelSocket)
	{
		...
		if (bBeamEnd)
		{
			// Does hit actor implement bullethit interface?
...
int32 AShooterCharacter::GetEmptyInventorySlot()
{
	...
	{
		if (Inventory[i] == nullptr) // Inventory has slot(s) in between
		{
			return i;
		}
	}
...
bool AShooterCharacter::GetIsInAir()
{
	if (AShooterCharacter::GetCharacterMovement()->IsFalling())
	{
		return true;
	}
	else
